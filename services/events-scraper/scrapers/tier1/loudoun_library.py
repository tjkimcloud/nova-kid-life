"""
Tier 1 scraper — Loudoun County Public Library events.
"""
...
class LoudounLibraryScraper(BaseScraper):
    ...
    def scrape(self) -> list[RawEvent]:
        ...
        try:
            ...
            logger.warning("[%s] LibCal failed: %s", self.source_name, exc)
        ...
        try:
            ...
            logger.warning("[%s] JSON-LD failed: %s", self.source_name, exc)
    ...
    def _scrape_libcal(self) -> list[RawEvent]:
        ...
        return [self._parse(item) for item in items if self._is_family(item)]

    def _is_family(self, item: dict) -> bool:
        family_keywords = {"kid", "child", "family", "teen", "toddler", "youth", "junior", "baby", "storytime"}
        text = (str(item.get("title", "")) + str(item.get("audience", ""))).lower()
        return any(kw in text for kw in family_keywords)

    def _parse(self, item: dict) -> RawEvent:
        ...
        return RawEvent(
            ...
            venue_name=venue,
            location_text=f"{venue}, Loudoun County, VA".strip(", "),
            is_free=True,
            event_type=EventType.EVENT,
            tags=["library", "loudoun"],
